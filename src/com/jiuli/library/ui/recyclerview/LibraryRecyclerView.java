...
/**
 * Created by siberiawolf on 15/12/22.
 */
public class LibraryRecyclerView extends RecyclerView implements Runnable{

    private static final String TAG = LibraryRecyclerView.class.getName();


    private Context mContext;

    private static LibraryRecyclerView recyclerView;
    ...
    public LibraryRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LibraryRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setHeaderImage(ImageView headerImage) {
        ...
        headerImageHeight = LibraryRecyclerView.this.headerImage.getHeight();
        // 防止第一次拉伸的时候headerImage.getLayoutParams().height = 0
        if (headerImageHeight <= 0) {
            headerImageHeight = LibraryRecyclerView.this.headerImage.getLayoutParams().height;
        } else {
    ...
    public void setLayoutManager(LayoutManager layout) {
        ...
        // 根据布局管理器设置分割线
        if (layout instanceof AnimLinearLayoutManager) {
            super.addItemDecoration(new DividerItemDecoration(mContext,
                    ((AnimLinearLayoutManager) layout).getOrientation(), true));
        } else {
    ...
    public void run() {
        ...
        if (manager instanceof AnimLinearLayoutManager) {
            // ListView布局
            ((AnimLinearLayoutManager) manager).setOverScrollListener(mOverScrollListener);
        } else if (manager instanceof AnimGridLayoutManager) {
            layoutGridAttach((AnimGridLayoutManager) manager);
        } else if (manager instanceof AnimStaggeredGridLayoutManager) {
            layoutStaggeredGridHeadAttach((AnimStaggeredGridLayoutManager) manager);
        }
     ...
     *
     */
    private void layoutStaggeredGridHeadAttach(AnimStaggeredGridLayoutManager manager) {
     ...
     *
     */
    private void layoutGridAttach(final AnimGridLayoutManager manager) {
        // GridView布局
        manager.setOverScrollListener(mOverScrollListener);
    ...
    public void onScrollStateChanged(int state) {
        ...
        // 当前不滚动，且不是正在刷新或加载数据
        if (state == RecyclerView.SCROLL_STATE_IDLE && onLoadDataListener != null && !isLoadingData) {
            LayoutManager layoutManager = getLayoutManager();
            int lastVisibleItemPosition;
            // 获取最后一个正在显示的Item的位置
            if (layoutManager instanceof AnimGridLayoutManager) {
                lastVisibleItemPosition = ((AnimGridLayoutManager) layoutManager).findLastVisibleItemPosition();
            } else if (layoutManager instanceof AnimStaggeredGridLayoutManager) {
                int[] into = new int[((AnimStaggeredGridLayoutManager) layoutManager).getSpanCount()];
                ((AnimStaggeredGridLayoutManager) layoutManager).findLastVisibleItemPositions(into);
                lastVisibleItemPosition = findMax(into);
            } else {
                lastVisibleItemPosition = ((AnimLinearLayoutManager) layoutManager).findLastVisibleItemPosition();
            }
    ...
    private void refresh() {
        if (headerImage.getLayoutParams().height >= headerImageMaxHeight
                && onLoadDataListener != null && !isLoadingData) {
            isLoadingData = true;
    ...
    private static class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 1:
                    ...
                    recyclerView.headerImage.setAlpha(1 - (float) msg.arg1 /
                            (recyclerView.headerImageMaxHeight - recyclerView.headerImageHeight) *
                            (1 - recyclerView.headerImageMinAlpha));
                    break;
            }
        }
        ...
        private void updateViewSize(Message msg) {
            // 重新设置View的宽高
            if (msg.obj != null) {
                recyclerView.headerImage.getLayoutParams().height += msg.arg1;
                View view = ((View) msg.obj);
                view.layout(view.getLeft(), 0, view.getRight(), view.getBottom());
            } else {
                // 实现类似弹簧的阻力效果，拉的越长就越难拉的动
                recyclerView.headerImageScaleHeight = recyclerView.headerImage.getLayoutParams().height
                        - recyclerView.headerImageHeight;
                if (recyclerView.headerImageScaleHeight < (recyclerView.headerImageMaxHeight - recyclerView.headerImageHeight) / 3) {
                    recyclerView.headerImage.getLayoutParams().height -= msg.arg1;
                } else if (recyclerView.headerImageScaleHeight > (recyclerView.headerImageMaxHeight - recyclerView.headerImageHeight) / 3 * 2) {
                    recyclerView.headerImage.getLayoutParams().height -= msg.arg1 / 3 * 2;
                } else {
                    recyclerView.headerImage.getLayoutParams().height -= msg.arg1 / 3 * 1.5;
                }
            }
            recyclerView.headerImage.requestLayout();
        }
    }
