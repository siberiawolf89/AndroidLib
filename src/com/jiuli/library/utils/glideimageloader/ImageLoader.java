...
public class ImageLoader {
    ...
    public static ImageLoadConfig defConfig = new ImageLoadConfig.Builder().
            ...
            setAsBitmap(true).
            setColorFilter(0).
            setPlaceHolderResId(R.drawable.bga_pp_ic_holder_dark).
            setErrorResId(R.drawable.bga_pp_ic_holder_dark).
    ...
    private static void load(Context context, ImageView view, Object objUrl, ImageLoadConfig config, final LoaderListener listener) {
        if (null == objUrl) {
            LibraryNotifyMgr.showShortToast("image url is null");
            return;
        } else if (null == config) {
            config = defConfig;
        }
        try {
            ...
            if (config.isAsGif()) {//gif类型
                ...
                if (config.isRoundedCorners()) {
                    ...
                    request.transform(new RoundedCornersTransformation(context, rounded, 0,cornerType));
                ...
                } else if (config.getColorFilter() != 0) {
                    request.transform(new ColorFilterTransformation(context, config.getColorFilter()));
                }
                builder = request;
